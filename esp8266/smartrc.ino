/*
 * ============================================================
 *  Smart RC Book — ESP8266 Firmware v3.1
 *
 *  Bug fixes in v3.1:
 *    Fix 1. Button event queue — simultaneous presses no longer lost
 *    Fix 2. All buttons now fire on PRESS (consistent feel)
 *    Fix 3. Forward declarations — safe compilation order
 *    Fix 4. resultFromCache flag — [C] badge only on actual cache hits
 *    Fix 5. drawNotFound shows tag UID on row 2 for debugging
 *    Fix 6. WiFi reconnect checks status before interrupting
 *    Fix 7. Non-blocking RFID poll — delay(50) replaced with millis()
 *    Fix 8. char[] buffers for url/tag — no heap fragmentation
 *
 *  Features from v3.0:
 *    1. Dedicated SELECT button on A0
 *    2. WiFi auto-reconnect + status indicator
 *    3. Fetch spinner animation
 *    4. Scan session counter
 *    5. Last-result cache (60s TTL)
 *    6. OLED sleep after 3 min inactivity
 *    7. Server reachability check on boot
 *
 *  ── WIRING ──────────────────────────────────────────────────
 *
 *  RC522 RFID → ESP8266
 *    SDA  → D8 (GPIO15)  SCK  → D5 (GPIO14)
 *    MOSI → D7 (GPIO13)  MISO → D6 (GPIO12)
 *    RST  → D3 (GPIO0)   3.3V/GND as usual
 *
 *  SH110X OLED (I2C) → ESP8266
 *    SDA → D2 (GPIO4)   SCL → D1 (GPIO5)
 *    VCC → 3.3V         GND → GND
 *
 *  BTN_NAV  → D0 (GPIO16)  10kΩ pull-up to 3.3V  active HIGH
 *  BTN_SEL  → A0           10kΩ pull-up to 3.3V  analogRead < 512
 *  BTN_BACK → D4 (GPIO2)   internal pull-up       active LOW
 *             ⚠ Do NOT hold during power-on
 *
 *  ── LIBRARIES ────────────────────────────────────────────────
 *    MFRC522  Adafruit SH110x  Adafruit GFX  ArduinoJson v6
 *    ESP8266WiFi  ESP8266HTTPClient  (both built-in)
 * ============================================================
 */
...
// ── STATE MACHINE ────────────────────────────────────────
enum State {
  STATE_WIFI, STATE_MENU, STATE_READY,
  STATE_FETCHING, STATE_RESULT, STATE_NOT_FOUND, STATE_ERROR
};
State currentState = STATE_WIFI;

// ── MENU ─────────────────────────────────────────────────
struct FieldGroup { const char* label; const char* fields; };
...
void enqueueBtn(int evt) {
  ...
  if (next != btnQHead) {          // don't overwrite if full
...
void drawWifi(int dots) {
  ...
  oledRow(1, WIFI_SSID);
  char d[4] = ""; for (int i = 0; i < dots && i < 3; i++) d[i] = '.';
  oledRow(2, d);
  display.display();
}
...
void drawResult() {
  ...
  for (int i = 0; i < RESULT_ROWS_OLED; i++) {
    ...
    for (int j = 0; j < 6 && k[j]; j++) k[j] = toupper(k[j]);
    char line[22];
...
void fetchRC(const char* tag) {
  ...
  if (strlen(cachedTag) > 0 &&
    ...
    Serial.print("Cache hit: "); Serial.println(tag);
  ...
  for (int i = 0, j = 0; fields[i] && j < 158; i++) {
    if (fields[i] == ',') {
      fieldsEnc[j++] = '%'; fieldsEnc[j++] = '2'; fieldsEnc[j++] = 'C';
    } else {
...
void setup() {
  ...
  if (!display.begin(0x3C, true)) {
    Serial.println("OLED not found"); while (true) delay(1000);
  }
  display.clearDisplay();
  display.setTextWrap(false);
  display.setTextSize(1);
  display.setTextColor(SH110X_WHITE);
  display.setCursor(10, 16); display.print("Smart RC Book v3.1");
  display.setCursor(22, 30); display.print("Starting...");
  display.display();
  delay(1000);
  ...
  if (WiFi.status() != WL_CONNECTED) {
    drawWifiFail(); delay(5000); ESP.restart();
  }
  ...
  switch (currentState) {
    ...
    case STATE_MENU:
      if (btn == 1) {
        ...
        Serial.print("Selected: "); Serial.println(MENU[menuIndex].label);
        goToReady();
      }
      else if (btn == 3) {
        if (menuIndex > 0) {
    ...
    case STATE_READY:
      if (btn == 3) { goToMenu(); break; }
      if (btn == 1 || btn == 2) { goToMenu(); break; }
    ...
    case STATE_RESULT:
      ...
      if (btn == 2) {
        if (resultScroll > 0) { resultScroll--; drawResult(); }
        break;
      }
      if (millis() - resultShowTime > RESULT_TIMEOUT_MS) goToReady();
      break;
